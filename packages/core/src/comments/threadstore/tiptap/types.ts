/**
 * Tiptap comment types have moved to a private tiptap package and we don't
 * want to create a dependency on it.
 * We've extracted the types from
 * https://github.com/ueberdosis/hocuspocus/blob/v2.15.3/packages/provider/src/types.ts
 * and added them here.
 */
...
export type DeleteCommentOptions = {
  /**
   * If true, the thread will also be deleted if the deleted comment was the
   * first comment in the thread.
   */
  deleteThread?: boolean;

  /**
   * If true, remove the content of the deleted comment.
   */
...
export type DeleteThreadOptions = {
  /**
   * If true, remove the comments on the thread. Otherwise mark the thread as
   * deleted and keep the comments.
   * @default false
   */
  deleteComments?: boolean;

  /**
   * If true, forcefully remove the thread and all comments. Otherwise mark the
   * thread as deleted and keep the comments.
   * @default false
   */
  force?: boolean;
};
